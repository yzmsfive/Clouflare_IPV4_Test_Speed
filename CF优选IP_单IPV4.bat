...
cls
title CF优选IP
echo 1. IPV4优选&echo 2. 自定义IPV4段&echo 3. 单IP测速&echo 4. 清空缓存&echo 0. 退出&echo.
set /p menu=请选择菜单:
if %menu%==0 exit
if %menu%==1 title IPV4优选&set ips=ipv4&set /a selfmode=0&goto bettercloudflareip
if %menu%==2 title 自定义IPV4段&set ips=ipv4&set /a selfmode=1&set /p selfip=请输入C类自定义IPV4(格式 104.16.16):&goto bettercloudflareip
if %menu%==3 title 单IP测速&call :singletest
if %menu%==4 del ipv4.txt ipv6.txt data.txt ip.txt CR.txt CRLF.txt cut.txt speed.txt meta.txt>nul 2>&1&RD /S /Q rtt>nul 2>&1
goto main
